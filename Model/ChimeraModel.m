function sol = ChimeraModel(params,domain,ins)
    ...
    for i=1:domain
        ...
        L_gamma_L = sum(L * ML_gamma_L) + sum(L(end-1: end));      
        L_j = L * ML_j;
        L_j = L_j(1:2:end) + L_j(2:2:end);
        L = L * ML;
        L(1:2) = [(1 - lambda_fq) * M(11, i) + lambda_qf * M(12, i), lambda_fq * M(11, i)...
            + (1 - lambda_qf) * M(12, i)] * (1 - vartheta_E_L) * (1 - exp( - phi_PL));
        
        M(7, i + 1) = sum(L(1:2:end));
        
        M(8, i + 1) = sum(L(2:2:end));
        ...
        JI_gamma_L = sum(JH * MJH_gamma_L);
        JI_mu = sum(JH * MJH_mu) + JH(end);
        JH = JH * MJH;
        JH(1) = (M(13, i) + M(22, i)) * (1 - exp( - phi_PH)) + (1 - exp( - phi_PH)) * (M(9, i) + M(10, i));%J2
        
        M(14, i + 1) = sum(JH);%JI_2 en adelante
        ...
        JA_gamma_L = sum(JL * MJL_gamma_L) + JL(end);
        JL = JL * MJL;
        JL(1) = (M(15, i) + M(23, i)) * (1 - exp( - phi_PL)) + vartheta_E_L * (1 - exp( - phi_PL)) * (M(11, i) + M(12, i));%%J_A2
        JL = JL + L_j;
        
        M(16, i + 1) = sum(JL);%JA_2 en adelante
